...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? 'onederful-dentistry-api-test/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from 'onederful-dentistry-api-test/_shims/index'`,
