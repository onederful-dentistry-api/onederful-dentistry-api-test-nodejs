...
import { APIUserAbortError } from 'onederful-dentistry-api-test';
import { Headers } from 'onederful-dentistry-api-test/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['ONEDERFUL_DENTISTRY_API_TEST_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['ONEDERFUL_DENTISTRY_API_TEST_BASE_URL'] = 'https://example.com/from_env';
      const client = new OnederfulDentistryAPITest({ accessToken: 'My Access Token' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['ONEDERFUL_DENTISTRY_API_TEST_BASE_URL'] = ''; // empty
      const client = new OnederfulDentistryAPITest({ accessToken: 'My Access Token' });
      expect(client.baseURL).toEqual('http://{{baseurl}}');
    });

    test('blank env variable', () => {
      process.env['ONEDERFUL_DENTISTRY_API_TEST_BASE_URL'] = '  '; // blank
      const client = new OnederfulDentistryAPITest({ accessToken: 'My Access Token' });
      expect(client.baseURL).toEqual('http://{{baseurl}}');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new OnederfulDentistryAPITest({ maxRetries: 4, accessToken: 'My Access Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new OnederfulDentistryAPITest({ accessToken: 'My Access Token' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new OnederfulDentistryAPITest({ accessToken: 'My Access Token', fetch: testFetch });
